    ...
    <div class="relative isolate overflow-hidden pt-20">
        <div class="mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <form method="post" action="{{ route('cars-location.update', $carLocation->id) }}" class="p-6">
                    @csrf
                    ...
                    <div class="mt-6">
                        ...
                        <x-select-input id="location_id" name="location_id" class="mt-1 block w-full"
                            :value="old('location_id')" required autofocus>
                            @foreach ($locations as $location)
                            <option value="{{ $location->id }}" @if($location->id ==
                                old('location_id',$carLocation->location_id))
                                selected @endif>
                    ...
                    <div class="mt-6">
                        ...
                        <x-select-input id="car_id" name="car_id" class="mt-1 block w-full" :value="old('car_id')"
                            required autofocus>
                            @foreach ($cars as $car)
                            <option value="{{ $car->id }}" @if($car->id == old('car_id',$carLocation->car_id))
                                selected @endif>
                                {{ $car->name }}</option>
                            @endforeach
                        </x-select-input>
                    ...
                    <div class="mt-6 grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="available_from" name="available_from" type="datetime-local"
                                class="mt-1 block w-full" :value="old('available_from',$carLocation->available_from)"
                        ...
                        <div>
                            ...
                            <x-text-input id="available_until" name="available_until" type="datetime-local"
                                class="mt-1 block w-full" :value="old('available_until',$carLocation->available_until)"
                                required autofocus autocomplete />
